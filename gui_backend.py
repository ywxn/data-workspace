"""Backend logic for the AI Data Workspace GUI."""
...
class DataWorkspaceBackend:
    ...
    def markdown_to_html(text: str) -> str:
        ...
        Delegates to the markdown_converter module for implementation.
    ...
    def load_file_data_with_ui(
        ...
        """Load and merge data from multiple files, returning dataframe and HTML welcome message."""
        ...
        if not dataframes:
            error_detail = "<br>".join(errors) if errors else "Unknown error"
            return (
                None,
                f"<p style='color: #cc0000;'>Failed to load any files:<br>{error_detail}</p>",
            )
        ...
        merge_info = (
            f"<p><b>Merge Strategy: </b>{merge_strategy}</p>" if merge_strategy else ""
        )
        file_word = "file" if len(file_info) == 1 else "files"

        welcome_msg = f"""<h3>Data Loaded Successfully</h3>
        <p><b>Loaded {len(file_info)} {file_word}:</b><br>{files_detail}</p>{merge_info}
        <p><b>Combined Shape:</b> {len(merged_dataframe)} rows, {len(merged_dataframe.columns)} columns</p>
        <p><b>Columns:</b> {columns_list}</p>"""

        if errors:
            error_detail = "<br>".join([f"&nbsp;&nbsp;• {err}" for err in errors])
            welcome_msg += (
                f"""<p style="color: #ff6600;"><b>Warnings:</b><br>{error_detail}</p>"""
            )

        welcome_msg += """<p style="color: #009900;">Ready to analyze your data! Try asking questions like:</p>
        <ul>
    ...
    def format_database_welcome_message(
        ...
        if isinstance(selected_tables, list):
            tables_detail = "<br>".join(
                [f"&nbsp;&nbsp;• {table}" for table in selected_tables]
            )
            merge_info = ""
            if "Merge strategy:" in status:
                merge_strategy = status.split("Merge strategy:")[1].strip()
                merge_info = f"<p><b>Merge Strategy: </b>{merge_strategy}</p>"

            table_word = "table" if len(selected_tables) == 1 else "tables"
            welcome_msg = f"""<h3>Data Loaded Successfully</h3>
            <p><b>Loaded from {db_type} database ({len(selected_tables)} {table_word}):</b><br>{tables_detail}</p>{merge_info}
            <p><b>Combined Shape:</b> {len(merged_dataframe)} rows, {len(merged_dataframe.columns)} columns</p>
            <p><b>Columns:</b> {columns_list}</p>
            <p style="color: #009900;">Ready to analyze your data! Try asking questions like:</p>
            <ul>
                <li>What insights can you find in this data?</li>
                <li>Show me a summary of the data</li>
                <li>What trends are visible?</li>
            ...
            welcome_msg = f"""<h3>Data Loaded Successfully</h3>
            <p><b>Loaded from {db_type} database</b></p>
            <p><b>Table:</b> {selected_tables}</p>
            <p><b>Shape:</b> {len(merged_dataframe)} rows, {len(merged_dataframe.columns)} columns</p>
            <p><b>Columns:</b> {columns_list}</p>
            <p style="color: #009900;">Ready to analyze your data! Try asking questions like:</p>
            <ul>
                <li>What insights can you find in this data?</li>
